        ...
        if extracted_db and os.path.exists(extracted_db):
            ...
            try:
                ...
                for table in tables:
                    ...
                    # Security: Validate table name to prevent SQL injection (must be a valid identifier)
                    if not table.isidentifier():
                        st.warning(f"  ⚠️ Skipped {table}: Invalid table name (possible SQL injection attempt)")
                        continue

                    try:
                        # Get table schema from source
                        # Safely quote the table name in addition to identifier check
