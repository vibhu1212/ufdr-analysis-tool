            ...
            try:
                ...
                for table in tables:
                    ...
                    if not table.isidentifier():
                        st.warning(f"  ⚠️ Skipped table with invalid name: {table}")
                        continue
                    ...
                    try:
                        ...
                        if rows:
                            ...
                            safe_columns_str = ','.join([f'"{col}"' for col in final_columns])

                            # Use OR IGNORE for cases to avoid unique constraint errors
                            conflict_clause = "OR IGNORE" if table == "cases" else "OR REPLACE"
                            insert_sql = f"INSERT {conflict_clause} INTO {safe_table} ({safe_columns_str}) VALUES ({placeholders})"
