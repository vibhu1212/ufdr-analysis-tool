...
## 2024-05-24 - Chat Interface Empty State
**Learning:** A blank canvas in a chat interface is confusing to users. Providing an empty state with example queries significantly helps users understand what they can ask and interact with.
**Action:** Implement helpful empty states with example actions/queries using `st.markdown` with explicit hex colors rather than relying on Streamlit css variables which might not render properly.
