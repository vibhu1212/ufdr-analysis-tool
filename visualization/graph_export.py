...
class GraphExporter:
    ...
    def export_anomaly_report(self, case_id, format='csv'):
        ...
        if not messages_df.empty:
            frames.append(messages_df[['timestamp', 'sender_digits', 'receiver_digits']])
        if not calls_df.empty:
            frames.append(calls_df[['timestamp', 'sender_digits', 'receiver_digits']])

        all_comms = pd.concat(frames, ignore_index=True)
        ...
        daily_stats = all_comms.groupby('date').agg({
            'timestamp': 'count',
            # Performance optimization: using native string identifier 'nunique'
            # instead of lambda x: len(set(x)) pushes calculation to C-level for a ~10-50x speedup
