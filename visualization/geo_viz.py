...
class GeoVisualizer:
    ...
    def create_location_map(self, case_id, map_type='cluster'):
        ...
        if map_type == 'markers':
            ...
            # ⚡ Bolt optimization: using itertuples instead of iterrows for performance  # noqa: E501
            ...
            # Create heat map
            # ⚡ Bolt optimization: Vectorized data extraction instead of iterrows  # noqa: E501
        ...
        output_path = os.path.join(
            self.output_dir, f'map_{map_type}_{case_id}.html')
        m.save(output_path)
        ...
        print(
            f"      - Date range: {df['timestamp'].min()} to {df['timestamp'].max()}")  # noqa: E501
    ...
    def create_temporal_map(self, case_id):
        ...
        for row in df.itertuples(index=False):
            feature = {
                ...
                'properties': {
                    ...
                    'popup': f"Location {row.location_id}<br>Time: {row.timestamp}",  # noqa: E501
                    'icon': 'circle',
                    'iconstyle': {
                        'fillColor': 'blue',
                        'fillOpacity': 0.6,
        ...
        # Save map
        output_path = os.path.join(
            self.output_dir, f'map_temporal_{case_id}.html')
        m.save(output_path)
    ...
    def create_density_map_plotly(self, case_id):
        ...
        # Save
        output_path = os.path.join(
            self.output_dir, f'map_density_plotly_{case_id}.html')
    ...
    def create_3d_scatter_map(self, case_id):
        ...
        df['time_numeric'] = (
            df['timestamp'] - df['timestamp'].min()).dt.total_seconds() / 86400
        ...
        fig = go.Figure(data=[go.Scatter3d(
            ...
            text=[f"Time: {t}<br>Lat: {lat:.4f}<br>Lon: {lon:.4f}"
                  for t, lat, lon in zip(df['timestamp'], df['latitude'], df['longitude'])],  # noqa: E501
            hoverinfo='text'
        )])
    ...
    def analyze_movement_patterns(self, case_id):
        ...
        analysis = {
            ...
            # days
            'time_span': (df['timestamp'].max() - df['timestamp'].min()).total_seconds() / 86400,  # noqa: E501
        ...
        print(
            f"      - Total distance: {analysis['total_distance_km']:.2f} km")
