        ...
        try:
            ...
            for record_type, type_records in grouped.items():
                ...
                for i in range(0, len(type_records), self.batch_size):
                    ...
                    # Group offsets by src_file to fetch only the needed ones
                    file_to_offsets = {}
                    for _, n in normalized_batch:
                        src_file = n.get('src_file')
                        src_offset = n.get('src_offset')
                        if src_file is not None and src_offset is not None:
                            file_to_offsets.setdefault(src_file, set()).add(src_offset)

                    existing_keys = set()

                    for src_file, offsets in file_to_offsets.items():
                        offsets_list = list(offsets)
                        # Process in chunks of 500 to avoid SQLite variable limits
                        for i in range(0, len(offsets_list), 500):
                            chunk = offsets_list[i:i+500]
                            placeholders = ','.join(['?' for _ in chunk])
                            query = f"SELECT src_file, src_offset FROM {table_name} WHERE case_id = ? AND src_file = ? AND src_offset IN ({placeholders})"
                            try:
                                cursor.execute(query, [case_id, src_file] + chunk)
                                for row in cursor.fetchall():
                                    existing_keys.add((row[0], row[1]))
                            except Exception as e:
                                logger.error(f"Error checking duplicates in bulk: {e}")

                    for record, normalized in normalized_batch:
                        try:
                            key = (normalized.get('src_file'), normalized.get('src_offset'))
